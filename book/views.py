...
def index_view(request):
    ...
    btn = request.GET.get('b')
    object_list = Book.objects.all()
    if searchForm.is_valid():
        keyword = searchForm.cleaned_data['keyword']
        if keyword:
            object_list = object_list.filter(title__contains=keyword)
        # object_list = Book.objects.filter(title__contains=keyword)
    if btn == "id":
        object_list = object_list.order_by('id')
    elif btn == "-id":
        object_list = object_list.order_by('-id')
    elif btn == "rank":
        object_list = object_list.annotate(avg_rating=Avg('review__rate')).order_by('-avg_rating')
    else:
        object_list = object_list.order_by('-id')
